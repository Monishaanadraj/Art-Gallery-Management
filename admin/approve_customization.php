...
if (isset($_GET['id'])) {
    ...
    $query = "UPDATE tblartcustomizations SET Status = 'Approved' WHERE ID = ?";
    ...
    if ($stmt) {
        ...
        if ($execute_result) {
            echo "<script>alert('Customization approved successfully.');</script>";
            echo "<script>window.location='customized-art.php';</script>";
        } else {
            echo "<script>alert('Error approving customization.');</script>";
            echo "<script>window.location='customized-art.php';</script>";
        }
